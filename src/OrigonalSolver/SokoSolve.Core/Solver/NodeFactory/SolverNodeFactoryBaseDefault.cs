...
namespace SokoSolve.Core.Solver.NodeFactory
{
    public abstract class SolverNodePoolingFactoryBaseDefault : SolverNodePoolingFactoryBase, ISolveNodePoolingFactoryPuzzleDependant
    {
        ...
        public override IBitmap CreateBitmap(IBitmap clone) => this.factoryClone?.Invoke(clone) ?? throw new InvalidOperationException("Factory not initialized");
        ...
        public override IBitmap CreateBitmap(VectorInt2 size) => this.factoryBySize?.Invoke(size) ?? throw new InvalidOperationException("Factory not initialized");
