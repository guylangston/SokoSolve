...
namespace SokoSolve.Core.Library
{
    ...
    public class TrivialNameValueFileFormat : Dictionary<string, string>
    {
        ...
        public static Tuple<string, string> SimpleSplit(string source, int position)
        {
            ...
            return new Tuple<string, string>(source.Substring(0, position), source.Substring(position + 1, source.Length - position - 1));
        }
        ...
        public const string Header = "# TrivialNameValueFileFormat (.tnv); lines in the form 'Property.Property.Name=Value<CRLF>'";
        ...
        public class WithBinder<T>
        {
            ...
            {
                return SetWhen(pair, default(T), selector, convert);
            }

            public WithBinder<T> SetWhen<TP>(KeyValuePair<string, string> pair, T target, Expression<Func<T, TP>> selector, Func<string, TP> convert = null)
            {
